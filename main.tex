...
\chapter{Test cases}
This section describes the results of the main tests done on the Appetite app.

\begin{itemize}
	\item Display event detail
	\item Display event list
	\item Display subscription list
	\item Display event map
	\item Create subscription
	\item Create event
	\item Disable notification for a sub.
	\item Enable notification for a sub.
	\item Delete event
	\item Delete subscription
	\item Vote event
	\item Undo vote event
	\item Login using Facebook
	\item Login using Google
	\item Register using email
	\item Login using email
	\item Reset email password
	\item Update events when moving map
